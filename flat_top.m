...
sinalB = A2*sin(2*pi*f2*t);%Segunda senoide
...
sinalCompostoFFT = fft(sinalComposto);%Passando o sinal para a frequencia
sinalFiltradoFFT = sinalCompostoFFT .* filtroIdeal;%Aplicando o filtro ideal
sinalFiltrado = real(ifft(sinalFiltradoFFT));%Voltando para o dominio do tempo
...
subplot(3,1,2);
stairs(t,sinalAmostradoFlatTop,'LineWidth',1.2);
grid on;
xlabel('Tempo (s)');
ylabel('Amplitude');
...
plot(t,sinalFiltrado,'--','LineWidth',1.2);
hold on;
stairs(t,sinalAmostradoFlatTop,'LineWidth',1.5);
grid on;
xlabel('Tempo (s)');
ylabel('Amplitude');
...
envolventeSinc = abs(sin(pi*fSinc*dReal)./(pi*fSinc*dReal));%Envolvente Sinc causada pela abertura do pulso
amplitudeMaxima = max(espectroMag);
envolventeSinc = envolventeSinc*amplitudeMaxima;

figure;

stem(f,espectroMag,'filled');
hold on;
...
subplot(3,1,2);
plot(f,filtroReconstrucaoPlot,'LineWidth',2);
grid on;
xlabel('Frequência (Hz)');
ylabel('Ganho');
...
subplot(3,1,3);
plot(t,sinalFiltrado,'LineWidth',2);
hold on;
plot(t,sinalReconstruido,'--','LineWidth',2);
grid on;
xlabel('Tempo (s)');
ylabel('Amplitude');
